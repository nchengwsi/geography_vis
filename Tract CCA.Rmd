...
```
# If you have not loaded the writexl pacakage from your library (the first code chunk in this rmd), run Librar(writexl) below. If you for some reason don't have it installed, you'll have to also run the install.packages("writexl") line below (or in first code chunk).
```{r}
install.packages("writexl")
library(writexl)
